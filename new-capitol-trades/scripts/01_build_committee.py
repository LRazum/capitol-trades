...
def main(args):
    ...
    import pandas as pd
    frames = []
    for path in args.assignments:  # e.g. House + Senate assignment files
        part = StewartWoonLoader.load(path, args.codes)
        print(f"  {Path(path).name}: {len(part)} rows")
        frames.append(part)
...
def parse_args():
    ...
    ap.add_argument("--assignments", required=True, nargs="+",
                    help="Stewart-Woon assignment file(s), e.g. the House and Senate files (.xls/.tab/.csv/.dta)")
