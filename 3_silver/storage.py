...
def latest_by_key(
    df: pd.DataFrame, key_col: str, recency_cols: Sequence[str]
) -> pd.DataFrame:
    """Keep one row per ``key_col`` — the one with the highest recency. Shared by the
    bronze upsert (per-partition) and the silver driver (global 'what's true now')."""
    rc = [c for c in recency_cols if c in df.columns]
    if rc:
...
class BronzeStore:
    ...
    def _dedupe(self, df: pd.DataFrame) -> pd.DataFrame:
        return latest_by_key(df, self.key_col, self.recency_cols)
